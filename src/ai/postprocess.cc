...
{
  /* 用 libc getline 替代逐字符 realloc（避免 O(n²) 内存抖动） */
  size_t cap = 0;
  ssize_t n = getline(&buffer, &cap, fp);
...
int readLines(const char *fileName, char *lines[], int max_line)
{
  ...
  {
    lines[i++] = s;
    s = NULL;  /* readLine 内部 getline 已分配，下次重新分配 */
    if (i >= max_line)
      break;
  }
  fclose(file);
                 ...
                 const char *labels_path)
{
  ...
  {
    int ret = 0;
    ret = loadLabelName(labels_path, labels);
    if (ret < 0)
    {
      return -1;
    }
  ...
  for (int i = 0; i < validCount; ++i)
  {
    ...
    /* labels 文件加载失败时为 NULL，用 "class_N" 兜底防段错误 */
    char *label = labels[id];
    if (NULL == label)
    {
      char fallback[32];
      snprintf(fallback, sizeof(fallback), "class_%d", id);
      strncpy(group->results[last_count].name, fallback, OBJ_NAME_MAX_SIZE - 1);
